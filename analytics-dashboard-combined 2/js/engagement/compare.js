...
const ComparePage = (function() {
  ...
  function renderCircularComparison() {
    ...
    const cellContent = (col, data, metrics, showVariance, percentileA, chartId, ctx) => `
      <div class="compare-cell compare-cell--${col}">
        ...
        <div class="compare-inner-section">
          <div class="compare-inner-section__header">
            <span class="material-symbols-outlined">analytics</span> Engagement Metrics
          </div>
          <div class="metric-rows">
            ${showVariance
              ? `${renderMetricRowWithVariance('Views', formatNumber(metrics.civ), metricsA.civ, metrics.civ)}
                 ${renderMetricRowWithVariance('Clicks', formatNumber(metrics.cc), metricsA.cc, metrics.cc)}
                 ${renderMetricRowWithVariance('Added', formatNumber(metrics.atl), metricsA.atl, metrics.atl)}`
              : `${renderMetricRow('Views', formatNumber(metrics.civ))}
                 ${renderMetricRow('Clicks', formatNumber(metrics.cc))}
                 ${renderMetricRow('Added', formatNumber(metrics.atl))}`
            }
          </div>
        </div>
        <div class="compare-inner-section">
          ...
          <div class="metric-rows">
            ${showVariance
              ? `${renderMetricRowWithVariance('Stores', formatNumber(metrics.storeCount), metricsA.storeCount, metrics.storeCount)}
                 ${renderMetricRowWithVariance('Categories', formatNumber(metrics.categoryCount), metricsA.categoryCount, metrics.categoryCount)}
                 ${renderMetricRowWithVariance('Promotions', formatNumber(metrics.promotionCount), metricsA.promotionCount, metrics.promotionCount)}`
              : `${renderMetricRow('Stores', formatNumber(metrics.storeCount))}
                 ${renderMetricRow('Categories', formatNumber(metrics.categoryCount))}
                 ${renderMetricRow('Promotions', formatNumber(metrics.promotionCount))}`
            }
          </div>
        </div>
      </div>`;
    ...
    const cellContent = (col, data, metrics, showVariance, percentileA, chartId, ctx) => `
      <div class="compare-cell compare-cell--${col}">
        ...
        <div class="compare-inner-section">
          ...
          <div class="metric-rows">
            ${showVariance
              ? `${renderMetricRowText('Card Size', metrics.cardSize || '-', metricsA.cardSize, metrics.cardSize)}
                 ${renderMetricRowWithVariance('Original Price', formatCurrency(metrics.originalPrice), metricsA.originalPrice, metrics.originalPrice)}
                 ${renderMetricRowWithVariance('Sale Price', formatCurrency(metrics.salePrice), metricsA.salePrice, metrics.salePrice)}`
              : `${renderMetricRow('Card Size', metrics.cardSize || '-')}
                 ${renderMetricRow('Original Price', formatCurrency(metrics.originalPrice))}
                 ${renderMetricRow('Sale Price', formatCurrency(metrics.salePrice))}`
            }
          </div>
        </div>
      </div>`;
  ...
  function renderPercentileRow(percentile, score, showVariance = false, percentileA = 0, metrics = null, chartId = null, ctx = null) {
    ...
    if (ctx) {
      ...
      const sentence = `Among the ${promoSpan} ${featuredPhrase}${categoryClause} during <span class="week">${week}</span> across <span class="store">${entity}</span>.<br>${subjectCap} scored in the <span class="percentile">${ordinal(percentile)} percentile</span>.`;
