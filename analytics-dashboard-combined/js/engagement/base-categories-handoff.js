 ...
 * Page-specific logic for engagement-categories.html
 */
...
(function() {
    ...
    const rows = pageCategories.map((cat, index) => {
      ...
      return `
        <tr class="category-row ${isSelected ? 'selected' : ''}"
          ...
          <td class="col-clicks">${core.formatNumber(cat.clicks)}</td>
      ...
      <table class="category-grid-table data-table--sortable ${moreDataClass}">
        <thead id="category-table-head">
          <tr>
            ...
            ${getCategorySortableHeaderHTML('Clicks', 'clicks', 'col-clicks')}
    ...
    detailContent.innerHTML = `
      ...
      <div class="category-metrics">
        ...
        <div class="category-metric">
          ...
          <div class="category-metric__value">${core.formatNumber(category.clicks)}</div>
        </div>
        <div class="category-metric">
          <div class="category-metric__label">Added to List</div>
          <div class="category-metric__value">${core.formatNumber(category.addToListCount)}</div>
        </div>
        <div class="category-metric">
  ...
  function viewCirculars() {
    core.saveState();
    StateManager.navigateTo('engagement-circulars.html', state);
  }
  ...
  function viewCategoryPromotions(categoryId) {
    ...
    if (!catId) return;
    core.saveState();
    StateManager.navigateTo('engagement-promotions.html', state, { categoryId: catId });
  }
  ...
  function openCategoryInquiry(categoryId) {
    ...
    if (!catId) return;
    core.saveState();
    StateManager.navigateTo('engagement-grid.html', state, { categoryId: catId });
  }
  ...
  function compareCategoryAction(categoryId) {
    ...
    if (!catId) return;
    core.saveState();
    StateManager.navigateTo('engagement-compare.html', state, { categoryId: catId });
  }
