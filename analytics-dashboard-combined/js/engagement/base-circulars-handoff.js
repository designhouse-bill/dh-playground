...
(function() {
    ...
    detailContent.innerHTML = `
      ...
      <div class="store-metrics">
        <div class="store-metric">
          ...
          <div class="store-metric__value">${core.formatNumber(store.views)}</div>
        </div>
        <div class="store-metric">
  ...
  function updatePaginationUI() {
    ...
    const pageInfoEl = document.getElementById('base-grid-page-info');
    if (pageInfoEl) {
      pageInfoEl.textContent = `Page ${pageInfo.currentPage} of ${pageInfo.totalPages || 1}`;
    }

    const rangeInfoEl = document.getElementById('base-grid-range-info');
    if (rangeInfoEl) {
      if (pageInfo.total === 0) {
    ...
    const prevBtn = document.getElementById('base-grid-prev-btn');
    if (prevBtn) {
      prevBtn.disabled = pageInfo.currentPage <= 1;
    }

    const nextBtn = document.getElementById('base-grid-next-btn');
    if (nextBtn) {
