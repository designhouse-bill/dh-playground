/**
 * Distribution Dashboard Feature
 * Ported from analytics-dashboard-distribution/features/distribution/distribution.js
 * loadHeader() removed — header is inlined in DOM.
 * Exposed as window.DistributionFeature for app.js.
 */

window.DistributionFeature = (function() {
  'use strict';
  ...
  function init() {
    ...
    try {
      ...
      initMap();
      isInitialized = true;
      console.log('✅ Distribution feature initialized');
    } catch (error) {
  ...
  function bindEvents() {
    document.querySelectorAll('#distribution-dashboard .section-nav__link').forEach(link => {
    ...
    // Scroll spy scoped to distribution section
    document.getElementById('distribution-dashboard')?.addEventListener('scroll', handleScrollSpy, { passive: true });
  ...
  function renderMediaKpis() {
    ...
    if (!m || !elements.mediaKpis) return;
  ...
  function renderCreativePanel() {
    if (!elements.creativeVariants) return;
  ...
  function renderVisitationKpis() {
    if (!elements.visitationKpis) return;
  ...
  function renderSpotlightCards() {
    if (!elements.spotlightCards) return;
  ...
  function renderTrafficKpis() {
    if (!elements.trafficKpis) return;
    const s = D.trafficShareMetrics.summary;
    elements.trafficKpis.innerHTML = [
      kpiTile(D.retailerConfig.label + ' Traffic Share', s.retailer_traffic_share + '%', { primary: true }),
  ...
  function renderConcentration() {
    if (!elements.concentrationStats) return;
    const s = D.trafficShareMetrics.summary;
  ...
  // ========================================
  // Store Map (lazy init handled in init())
  // ========================================

  function initMap() {
    if (mapInitialized || typeof StoreMap === 'undefined') return;
  ...
  function handleSectionNavClick(e) {
    ...
    document.querySelectorAll('#distribution-dashboard .section-nav__link').forEach(l => l.classList.remove('active'));
    e.currentTarget.classList.add('active');
  }

  function handleScrollSpy() {
    if (document.getElementById('distribution-dashboard')?.classList.contains('active') === false) return;
    const sections = document.querySelectorAll('.dist-section');
    const navLinks = document.querySelectorAll('#distribution-dashboard .section-nav__link');
    let currentSection = '';
  ...
  // ========================================
  // Filter Chip Handler (called from app.js)
  // ========================================
  ...
  function updateFilterChips() {
    ...
    if (ctx.flightWeek !== 'all') {
      const wk = D.flightWeeks.find(function(w) { return w.id === ctx.flightWeek; });
  ...
  // Listen for data refresh from modals
