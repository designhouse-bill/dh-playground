/**
 * DefaultWeeklyReport.js
 * Comprehensive weekly report with top/bottom performers, YTD, and detailed metrics
 */

class DefaultWeeklyReport {
    ...
    calculateKPIs(promotions, week) {
        ...
        // Category stats with top/bottom performers
        const categoryStats = {};
        promotions.forEach(p => {
            const category = p.marketing_category || p.department || 'Unknown';
            if (!categoryStats[category]) {
                categoryStats[category] = { views: 0, clicks: 0, count: 0 };
            }
        ...
        // Top/bottom promotions by composite score with full details
        const sortedPromotions = promotions
            .sort((a, b) => (b.composite_score || 0) - (a.composite_score || 0));

        const topPromotions = sortedPromotions.slice(0, 3)
            .map(p => ({
                name: p.card_name,
                ...
                addedToList: p.added_to_list || 0,
                ctr: p.card_in_view > 0 ? ((p.card_clicked / p.card_in_view) * 100).toFixed(1) : 0
            }));

        const bottomPromotions = sortedPromotions.slice(-3).reverse()
            .map(p => ({
                name: p.card_name,
                score: p.composite_score,
                dealType: p.deal_type || 'N/A',
        ...
        promotions.forEach(p => {
            ...
            if (storeId) {
                ...
                if (!groupStats[groupId]) {
                    groupStats[groupId] = { views: 0, clicks: 0, count: 0 };
                }
                groupStats[groupId].views += (p.card_in_view || 0);
                groupStats[groupId].clicks += (p.card_clicked || 0);
                groupStats[groupId].count += 1;

                // Store stats
                if (!storeStats[storeId]) {
                    storeStats[storeId] = { views: 0, clicks: 0, count: 0 };
                }
                storeStats[storeId].views += (p.card_in_view || 0);
                storeStats[storeId].clicks += (p.card_clicked || 0);
            ...
            if (!versionStats[version]) {
                versionStats[version] = { views: 0, clicks: 0, count: 0 };
            }
            versionStats[version].views += (p.card_in_view || 0);
            versionStats[version].clicks += (p.card_clicked || 0);
            versionStats[version].count += 1;
        });

        // Top/bottom groups
        const sortedGroups = Object.entries(groupStats).sort((a, b) => b[1].views - a[1].views);
        const topGroups = sortedGroups.slice(0, 3).map(([name, stats]) => ({ name, ...stats }));
        const bottomGroups = sortedGroups.slice(-3).reverse().map(([name, stats]) => ({ name, ...stats }));

        // Top/bottom stores (if multiple stores)
        const sortedStores = Object.entries(storeStats).sort((a, b) => b[1].views - a[1].views);
        const topStores = sortedStores.length > 3 ? sortedStores.slice(0, 3).map(([name, stats]) => ({ name, ...stats })) : [];
        const bottomStores = sortedStores.length > 3 ? sortedStores.slice(-3).reverse().map(([name, stats]) => ({ name, ...stats })) : [];

        // Top/bottom versions (if multiple versions)
        const sortedVersions = Object.entries(versionStats).sort((a, b) => b[1].views - a[1].views);
        const topVersions = sortedVersions.length > 1 ? sortedVersions.slice(0, 3).map(([name, stats]) => ({ name, ...stats })) : [];
        const bottomVersions = sortedVersions.length > 1 ? sortedVersions.slice(-3).reverse().map(([name, stats]) => ({ name, ...stats })) : [];
        ...
        const interactionRate = totalViews > 0 ? ((totalClicks + totalAddedToList + totalShares) / totalViews * 100) : 0;

        // YTD metrics (if available)
        const ytdMetrics = window.mockDatabase?.ytdMetrics || null;

        // Daily trend (mock 7 days of data)
        ...
        return {
            ...
            // Top/bottom stores (if applicable)
            topStores,
            bottomStores,

            // Top/bottom versions (if applicable)
            topVersions,
            bottomVersions,
        ...
        return `
            <div style="padding: 1rem; background: var(--bg); min-height: 100%;">
                <div style="margin-bottom: 1rem;">
                    <h3 style="margin: 0 0 0.25rem 0; font-size: 18px; font-weight: 700;">Default Weekly Report</h3>
                ...
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.75rem; margin-bottom: 1rem;">
                    ...
                    <div class="card" style="padding: 1rem;">
                        ...
                        <div style="font-size: 24px; font-weight: 700; color: var(--text); margin-bottom: 0.25rem;">${kpis.totalViews.toLocaleString()}</div>
                        <div style="font-size: 11px; color: ${kpis.viewsChange >= 0 ? 'var(--green)' : 'var(--red)'};">
                            ${kpis.viewsChange >= 0 ? '↑' : '↓'} ${Math.abs(kpis.viewsChange).toFixed(1)}% vs last week
                        </div>
                        <div style="font-size: 10px; color: var(--muted); margin-top: 0.25rem;">Across ${kpis.promotionCount} promotions</div>
                    </div>

                    <!-- Total Clicks -->
                    <div class="card" style="padding: 1rem;">
                        <div style="font-size: 11px; color: var(--muted); text-transform: uppercase; margin-bottom: 0.5rem;">Total Clicks</div>
                        <div style="font-size: 24px; font-weight: 700; color: var(--text); margin-bottom: 0.25rem;">${kpis.totalClicks.toLocaleString()}</div>
                        <div style="font-size: 11px; color: ${kpis.clicksChange >= 0 ? 'var(--green)' : 'var(--red)'};">
                            ${kpis.clicksChange >= 0 ? '↑' : '↓'} ${Math.abs(kpis.clicksChange).toFixed(1)}% vs last week
                        </div>
                        <div style="font-size: 10px; color: var(--muted); margin-top: 0.25rem;">CTR: ${kpis.ctr.toFixed(2)}%</div>
                    </div>

                    <!-- Best Performer -->
                    <div class="card" style="padding: 1rem;">
                        <div style="font-size: 11px; color: var(--muted); text-transform: uppercase; margin-bottom: 0.5rem;">Best Performing Group</div>
                        <div style="font-size: 16px; font-weight: 600; color: var(--text); margin-bottom: 0.25rem;">${kpis.bestPerformer?.name || 'N/A'}</div>
                        <div style="font-size: 11px; color: var(--muted);">${(kpis.bestPerformer?.views || 0).toLocaleString()} views</div>
                    </div>
                </div>
                ...
                <div class="card" style="padding: 1rem; margin-bottom: 0.75rem;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.75rem;">
                        ${kpis.topPromotions.map((promo, index) => `
                            <div style="padding: 0.75rem; background: var(--bg); border-radius: 6px;">
                                <div style="font-size: 10px; color: var(--muted); margin-bottom: 0.25rem;">#${index + 1} • Score: ${promo.score.toFixed(0)}</div>
                                <div style="font-size: 13px; font-weight: 600; color: var(--text); margin-bottom: 0.5rem; line-height: 1.3; height: 2.6em; overflow: hidden;">${promo.name}</div>
                                <div style="display: grid; gap: 0.25rem; font-size: 10px; color: var(--muted);">
                                    ...
                                    <div style="margin-top: 0.25rem; padding-top: 0.25rem; border-top: 1px solid var(--border);">
                                        ${promo.views.toLocaleString()} views • ${promo.clicks.toLocaleString()} clicks • ${promo.ctr}% CTR
                                    </div>
                                </div>
                            </div>
                        `).join('')}
                ...
                <!-- Daily Trend Chart -->
                <div class="card" style="padding: 1rem;">
                    <div style="font-size: 11px; color: var(--muted); text-transform: uppercase; margin-bottom: 0.75rem;">Daily Views Trend</div>
                    ${this.generateChartSVG(kpis.dailyViews, 368, 80)}
                    <div style="display: flex; justify-content: space-between; margin-top: 0.5rem; font-size: 10px; color: var(--muted);">
                        <span>Mon</span>
                        <span>Tue</span>
                        <span>Wed</span>
                        <span>Thu</span>
...
window.DefaultWeeklyReport = DefaultWeeklyReport;