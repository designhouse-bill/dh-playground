...
(function() {
  ...
  const elements = {
    ...
    segmentBtns: document.querySelectorAll('.v6-segment-btn'),
  ...
  function createCategoryItem(cat) {
    ...
    const allClass = cat.isAll ? 'v6-category-item--all' : '';
    ...
    if (state.comparisonEnabled && !cat.isAll && cat.id !== 'all') {
      ...
      if (comparison && comparison.change) {
        comparisonHTML = `
          <div class="v6-category-item__comparison">
    ...
    return `
      <div class="v6-category-item ${allClass} ${isActive ? 'active' : ''}"
           data-id="${cat.id}"
           role="listitem"
           tabindex="0"
           aria-label="${cat.name}, ${cat.promotionCount} promotions">
        <div class="v6-category-item__info">
          <div class="v6-category-item__name">${escapeHtml(cat.name)}</div>
          <div class="v6-category-item__meta">
  ...
  function renderCategoryGrid() {
    ...
    const rows = categories.map((cat, index) => {
      ...
      return `
        <tr class="v6-category-row ${isSelected ? 'selected' : ''}"
            data-category-id="${cat.id}"
            onclick="selectCategoryGridRow('${cat.id}')">
          <td class="v6-col-num">
            <span class="v6-table-position">${index + 1}</span>
          </td>
          <td class="v6-col-category">
            <div class="v6-category-name">
              <div class="v6-category-icon">
                <span class="material-symbols-outlined">category</span>
              </div>
              <span class="v6-category-label">${escapeHtml(cat.name)}</span>
            </div>
          </td>
          <td class="v6-col-perf v6-table-performance">
            <div class="v6-perf-bar v6-table-bar--wide">
              <div class="v6-perf-bar__track">
                <div class="v6-perf-bar__fill v6-perf-bar__fill--${perfClass}" style="width: ${perfPercent}%"></div>
              </div>
              <span class="v6-perf-bar__value">${formatNumber(cat.compositeScore)}</span>
            </div>
          </td>
          <td class="v6-col-metric v6-table-metric">${formatNumber(cat.civ)}</td>
          <td class="v6-col-metric v6-table-metric">${formatNumber(cat.cc)}</td>
          <td class="v6-col-metric v6-table-metric">${formatNumber(cat.atl)}</td>
          <td class="v6-col-metric v6-table-metric">${cat.percentile}%</td>
        </tr>
      `;
    });

    gridContainer.innerHTML = `
      <table class="v6-data-table v6-data-table--sortable">
        <thead>
          <tr>
            <th class="v6-col-num">#</th>
            ${getCategorySortableHeaderHTML('Category', 'name', 'v6-col-category')}
            ${getCategorySortableHeaderHTML('Performance', 'compositeScore', 'v6-col-perf v6-th-performance')}
            ${getCategorySortableHeaderHTML('Views', 'civ', 'v6-col-metric')}
            ${getCategorySortableHeaderHTML('Clicks', 'cc', 'v6-col-metric')}
            ${getCategorySortableHeaderHTML('Added', 'atl', 'v6-col-metric')}
            ${getCategorySortableHeaderHTML('%tile', 'percentile', 'v6-col-metric')}
          </tr>
        </thead>
  ...
  function selectCategoryGridRow(categoryId) {
    ...
    const rows = document.querySelectorAll('.v6-category-row');
    rows.forEach(row => {
    ...
    if (elements.categoryDetailPanel) {
      elements.categoryDetailPanel.classList.remove('v6-panel--collapsed');
      elements.categoryDetailPanel.classList.add('v6-panel--expanded');
    }
  }
  ...
  function renderCategoryDetail(categoryId) {
    ...
    elements.categoryDetailContent.innerHTML = `
      <div class="v6-category-detail-header">
        <h3 class="v6-category-detail-name">${escapeHtml(category.name)}</h3>
        <div class="v6-category-detail-count">
      ...
      <div class="v6-category-metrics">
        <div class="v6-category-metric">
          <div class="v6-category-metric__label">Views</div>
          <div class="v6-category-metric__value">${formatNumber(category.civ)}</div>
        </div>
        <div class="v6-category-metric">
          <div class="v6-category-metric__label">Clicks</div>
          <div class="v6-category-metric__value">${formatNumber(category.cc)}</div>
        </div>
        <div class="v6-category-metric">
          <div class="v6-category-metric__label">Added to List</div>
          <div class="v6-category-metric__value">${formatNumber(category.atl)}</div>
        </div>
        <div class="v6-category-metric">
          <div class="v6-category-metric__label">Percentile</div>
          <div class="v6-category-metric__value">${category.percentile}%</div>
        </div>
      </div>

      <div class="v6-category-actions">
        <button class="v6-category-action-btn v6-category-action-btn--primary" onclick="viewCategoryPromotions('${categoryId}')">
          <span class="material-symbols-outlined">visibility</span>
          View Promotions (${category.promotionCount})
        </button>
        <button class="v6-category-action-btn v6-category-action-btn--secondary" onclick="openCategoryInquiry('${categoryId}')">
          <span class="material-symbols-outlined">table_chart</span>
          Inquiry Data Grid
        </button>
        <button class="v6-category-action-btn v6-category-action-btn--secondary" onclick="compareCategoryAction('${categoryId}')">
  ...
  function closeCategoryDetail() {
    ...
    if (elements.categoryDetailPanel) {
      elements.categoryDetailPanel.classList.add('v6-panel--collapsed');
      elements.categoryDetailPanel.classList.remove('v6-panel--expanded');
    }
  }
  ...
  function createPromoCard(promo, position, maxValues) {
    ...
    if (state.comparisonEnabled) {
      ...
      if (comparison && comparison.change) {
        comparisonHTML = `
          <div class="v6-promo-card__comparison">
    ...
    return `
      <div class="v6-promo-card ${isActive ? 'active' : ''}"
           data-id="${promo.id}"
           role="listitem"
           tabindex="0"
           aria-label="${promo.name}">
        <div class="v6-promo-card__image-wrapper">
          <div class="v6-promo-card__position">${position}</div>
          <img class="v6-promo-card__image"
               src="${promo.thumbImage}"
               alt="${escapeHtml(promo.name)}"
               loading="lazy">
        </div>
        <div class="v6-promo-card__content">
          <div class="v6-promo-card__badges">
            <span class="v6-promo-card__deal-badge">${escapeHtml(promo.dealType)}</span>
            ${getPercentileBadgeHTML(promo.percentile)}
          </div>
          <h3 class="v6-promo-card__title">${escapeHtml(promo.name)}</h3>
          <p class="v6-promo-card__meta">${escapeHtml(promo.categoryName)}</p>
          <div class="v6-promo-card__stats">
            <div class="v6-promo-stat">
              <span class="v6-promo-stat__value">${formatNumber(promo.civ)}</span>
              <span class="v6-promo-stat__label">Views</span>
            </div>
            <div class="v6-promo-stat">
              <span class="v6-promo-stat__value">${formatNumber(promo.cc)}</span>
              <span class="v6-promo-stat__label">Clicks</span>
            </div>
            <div class="v6-promo-stat">
              <span class="v6-promo-stat__value">${formatNumber(promo.atl)}</span>
              <span class="v6-promo-stat__label">Added</span>
            </div>
          </div>
          <div class="v6-promo-card__chart">
            <div class="v6-promo-bar">
              <span class="v6-promo-bar__label">Perf</span>
              <div class="v6-promo-bar__track">
                <div class="v6-promo-bar__fill v6-promo-bar__fill--views" style="width: ${performanceWidth}%"></div>
              </div>
            </div>
          </div>
          ${comparisonHTML}
    ...
    const rows = displayPromotions.map((promo, index) => {
      ...
      return `
        <tr class="${isActive ? 'active' : ''}" data-id="${promo.id}">
          <td>
            <div class="v6-table-position">${index + 1}</div>
          </td>
          <td>
            <div class="v6-table-promo">
              <img class="v6-table-thumb" src="${promo.thumbImage}" alt="${escapeHtml(promo.name)}">
              <div class="v6-table-info">
                <div class="v6-table-title">${escapeHtml(promo.name)}</div>
                <div class="v6-table-meta">${escapeHtml(promo.categoryName)}</div>
              </div>
            </div>
          </td>
          <td class="v6-table-performance">
            <div class="v6-table-bar v6-table-bar--wide">
              <div class="v6-table-bar__track">
                <div class="v6-table-bar__fill" style="width: ${performanceWidth}%"></div>
              </div>
            </div>
          </td>
          <td><span class="v6-table-deal">${escapeHtml(promo.dealType)}</span></td>
          <td class="v6-table-metric">${formatNumber(promo.civ)}</td>
          <td class="v6-table-metric">${formatNumber(promo.cc)}</td>
          <td class="v6-table-metric">${formatNumber(promo.atl)}</td>
      ...
      <table class="v6-data-table v6-data-table--sortable">
        <thead>
          <tr>
            ...
            <th class="v6-th-performance">Performance</th>
  ...
  function renderDetail(promo) {
    ...
    if (state.comparisonEnabled) {
      ...
      if (comparison && comparison.change) {
        ...
        const sectionClass = isPositive ? '' : 'v6-detail-comparison--negative';

        comparisonSectionHTML = `
          <div class="v6-detail-comparison ${sectionClass}">
            <div class="v6-detail-comparison__header">
              <div class="v6-detail-comparison__title">Week-over-Week Performance</div>
              <div class="v6-detail-comparison__period">Week 47 vs. Week 46</div>
            </div>
            <div class="v6-detail-comparison__metrics">
              <div class="v6-detail-comparison__metric">
                <div class="v6-detail-comparison__metric-label">Card in View</div>
                <div class="v6-detail-comparison__metric-value">
                  ${getComparisonArrowHTML(comparison.change.civ)}
                  ${MockData.formatChange(comparison.change.civ)}
                </div>
              </div>
              <div class="v6-detail-comparison__metric">
                <div class="v6-detail-comparison__metric-label">Card Clicked</div>
                <div class="v6-detail-comparison__metric-value">
              ...
              <div class="v6-detail-comparison__metric">
                <div class="v6-detail-comparison__metric-label">Add to List</div>
                <div class="v6-detail-comparison__metric-value">
              ...
              <div class="v6-detail-comparison__metric">
                <div class="v6-detail-comparison__metric-label">Score</div>
                <div class="v6-detail-comparison__metric-value">
    ...
    elements.detailContent.innerHTML = `
      <div class="v6-detail-hero">
        <img src="${promo.heroImage}" alt="${escapeHtml(promo.name)}">
      </div>
      <div class="v6-detail-body">
        <h2 class="v6-detail-title">${escapeHtml(promo.name)}</h2>
        <p class="v6-detail-meta">
          ${escapeHtml(promo.categoryName)} • ${escapeHtml(promo.dealType)} • ${escapeHtml(promo.cardSize)}
        </p>

        <div class="v6-detail-kpis">
          <div class="v6-detail-kpi">
            <span class="v6-detail-kpi__value">${formatNumber(promo.civ)}</span>
            <span class="v6-detail-kpi__label">Card in View</span>
          </div>
          <div class="v6-detail-kpi">
            <span class="v6-detail-kpi__value">${formatNumber(promo.cc)}</span>
            <span class="v6-detail-kpi__label">Card Clicked</span>
          </div>
          <div class="v6-detail-kpi">
            <span class="v6-detail-kpi__value">${formatNumber(promo.atl)}</span>
            <span class="v6-detail-kpi__label">Add to List</span>
          </div>
          <div class="v6-detail-kpi">
            <span class="v6-detail-kpi__value">${ctr}%</span>
            <span class="v6-detail-kpi__label">Click-Through Rate</span>
          </div>
        </div>
        ...
        <div class="v6-detail-section">
          <div class="v6-detail-section__title">Performance</div>
        ...
        <div class="v6-detail-section">
          <div class="v6-detail-section__title">7-Day Trend</div>
          <div class="v6-mini-chart" id="trend-chart"></div>
        </div>

        <div class="v6-detail-section">
          <div class="v6-detail-section__title">Top Performing Stores</div>
          <div class="v6-store-list">
            ${MockData.topStores.slice(0, 5).map(store => `
              <div class="v6-store-item">
                <span class="v6-store-item__name">${escapeHtml(store.name)}</span>
                <span class="v6-store-item__score">${store.score}</span>
              </div>
            `).join('')}
          </div>
        </div>
  ...
  function renderEmptyDetail() {
    ...
    elements.detailContent.innerHTML = `
      <div class="v6-detail-empty">
        <div class="v6-detail-empty__icon" aria-hidden="true">👆</div>
        <p class="v6-detail-empty__text">Select a promotion to view details</p>
      </div>
    `;
  }
  ...
  function renderLoadingState() {
    const loadingHTML = `
      <div class="v6-loading-state">
        <div class="v6-spinner"></div>
        <p class="v6-loading-text">Loading data...</p>
      </div>
    `;
  ...
  function renderErrorState(error) {
    const errorHTML = `
      <div class="v6-error-state">
        <div class="v6-error-icon" aria-hidden="true">⚠️</div>
        <h3 class="v6-error-title">Failed to Load Data</h3>
        <p class="v6-error-message">${escapeHtml(error.message || 'An unexpected error occurred')}</p>
        <button class="btn btn--primary v6-retry-btn" onclick="location.reload()">
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('.v6-view-toggle__btn').forEach(btn => {
    ...
    document.querySelectorAll('.v6-modal-overlay').forEach(overlay => {
    ...
    const closeBtn = document.querySelector('.v6-panel-close');
    if (closeBtn) {
  ...
  function closeDetailPanel() {
    ...
    if (elements.detailPanel) {
      elements.detailPanel.classList.remove('v6-panel--expanded');
      elements.detailPanel.classList.add('v6-panel--collapsed');
    }
  ...
  function handleCategoryClick(e) {
    const item = e.target.closest('.v6-category-item');
    if (!item) return;
  ...
  function handlePromotionClick(e) {
    const card = e.target.closest('.v6-promo-card');
    if (!card) return;
  ...
  function handlePromoViewToggle(e) {
    const btn = e.target.closest('.v6-view-toggle__btn');
    if (!btn) return;
    ...
    document.querySelectorAll('.v6-view-toggle__btn').forEach(b => {
  ...
  async function selectPromotion(promoId) {
    ...
    // Expand the detail panel
    if (elements.detailPanel) {
      elements.detailPanel.classList.remove('v6-panel--collapsed');
      elements.detailPanel.classList.add('v6-panel--expanded');
    }
    ...
    try {
      ...
      if (elements.detailContent) {
        elements.detailContent.innerHTML = `
          <div class="v6-loading-state">
            <div class="v6-spinner"></div>
            <p class="v6-loading-text">Loading details...</p>
          </div>
        `;
      }
      ...
      const detailPanel = document.querySelector('.v6-panel--right');
      ...
      if (elements.detailContent) {
        elements.detailContent.innerHTML = `
          <div class="v6-error-state">
            <div class="v6-error-icon" aria-hidden="true">⚠️</div>
            <h3 class="v6-error-title">Failed to Load Details</h3>
            <p class="v6-error-message">${escapeHtml(error.message)}</p>
  ...
  function handleGlobalKeydown(e) {
    ...
    if (e.key === 'Escape') {
      ...
      document.querySelectorAll('.v6-modal-overlay.active').forEach(overlay => {
  ...
  function updateDateDisplay() {
    ...
    const valueEl = dateCard.querySelector('.v6-card-value');
    const subEl = dateCard.querySelector('.v6-card-sub');
  ...
  function updateEntityDisplay() {
    ...
    const valueEl = entityCard.querySelector('.v6-card-value');
    const subEl = entityCard.querySelector('.v6-card-sub');
    const breadcrumbEl = entityCard.querySelector('.v6-card-breadcrumb');
    const iconEl = entityCard.querySelector('.v6-card-icon .material-symbols-outlined');
  ...
  function renderWeekList(searchQuery = '') {
    ...
    const weeksHTML = filteredWeeks.map(week => {
      ...
      return `
        <div class="v6-week-option ${isSelected ? 'selected' : ''}"
             data-week-id="${week.id}"
             onclick="selectWeek('${week.id}')">
          <div class="v6-week-option-left">
            <div class="v6-week-option-label">${week.label}</div>
            <div class="v6-week-option-dates">${week.dateRange}</div>
          </div>
          <div class="v6-week-option-check">
    ...
    elements.weekList.innerHTML = weeksHTML || '<div class="v6-week-option" style="text-align:center;color:var(--color-text-tertiary);">No weeks found</div>';
  }
  ...
  function renderEntityTree(searchQuery = '') {
    ...
    if (matchesSearch('All') || !query) {
      html += `
        <tr class="v6-tree-row--brand ${isAllSelected ? 'selected' : ''}"
            data-entity-id="all"
            data-level="all"
            data-parent="">
          <td class="v6-tree-indent-0">
            <div class="v6-tree-name-cell">
              <button class="v6-tree-toggle ${isAllExpanded ? '' : 'collapsed'}" onclick="toggleTreeRow('all', event)">
                <span class="material-symbols-outlined">expand_more</span>
              </button>
              <span onclick="selectTreeEntity('all', 'all', 'All Stores', ${totalStoreCount})">All Stores</span>
            </div>
          </td>
          <td><span class="v6-type-badge v6-type-badge--brand">All</span></td>
          <td></td>
          <td>all</td>
    ...
    MockData.entities.brands.forEach(brand => {
      ...
      const brandHidden = !isAllExpanded ? 'v6-tree-row-hidden' : '';

      if (matchesSearch(brand.name) || !query) {
        html += `
          <tr class="v6-tree-row--brand ${isBrandSelected ? 'selected' : ''} ${brandHidden}"
              data-entity-id="${brand.id}"
              data-level="brand"
              data-parent="all">
            <td class="v6-tree-indent-1">
              <div class="v6-tree-name-cell">
                <button class="v6-tree-toggle ${isBrandExpanded ? '' : 'collapsed'}" onclick="toggleTreeRow('${brand.id}', event)">
                  <span class="material-symbols-outlined">expand_more</span>
                </button>
                <span onclick="selectTreeEntity('${brand.id}', 'brand', '${escapeHtml(brand.name)}', ${brand.storeCount})">${escapeHtml(brand.name)}</span>
              </div>
            </td>
            <td><span class="v6-type-badge v6-type-badge--brand">Brand</span></td>
            <td></td>
            <td>${brand.id}</td>
      ...
      brandSubBrands.forEach(subBrand => {
        ...
        if (subBrandMatches || hasMatchingChildren || !query) {
          ...
          const subHidden = (!isAllExpanded || !isBrandExpanded) ? 'v6-tree-row-hidden' : '';

          html += `
            <tr class="v6-tree-row--subbrand ${isSubSelected ? 'selected' : ''} ${subHidden}"
                data-entity-id="${subBrand.id}"
                data-level="subbrand"
                data-parent="${brand.id}">
              <td class="v6-tree-indent-2">
                <div class="v6-tree-name-cell">
                  <button class="v6-tree-toggle ${isSubExpanded ? '' : 'collapsed'}" onclick="toggleTreeRow('${subBrand.id}', event)">
                    <span class="material-symbols-outlined">expand_more</span>
                  </button>
              ...
              <td><span class="v6-type-badge v6-type-badge--subbrand">SubBrand</span></td>
              <td></td>
              <td>${subBrand.id}</td>
          ...
          storesToShow.forEach(store => {
            ...
            const storeHidden = (!isAllExpanded || !isBrandExpanded || !isSubExpanded) ? 'v6-tree-row-hidden' : '';

            html += `
              <tr class="v6-tree-row--store ${isStoreSelected ? 'selected' : ''} ${storeHidden}"
                  data-entity-id="${store.id}"
                  data-level="store"
                  data-parent="${subBrand.id}">
                <td class="v6-tree-indent-3">
                  <div class="v6-tree-name-cell">
                    <span class="v6-tree-toggle-placeholder"></span>
                    <span onclick="selectTreeEntity('${store.id}', 'store', '${escapeHtml(store.title)}', 1)">${escapeHtml(store.name)}</span>
                  </div>
                </td>
                <td><span class="v6-type-badge v6-type-badge--store">Store</span></td>
  ...
  function selectTreeEntity(entityId, level, name, count) {
    ...
    document.querySelectorAll('.v6-group-item').forEach(el => {
  ...
  function renderFilterOptions() {
    ...
    if (categoryOptions && state.categories) {
      const html = state.categories.map(cat =>
        `<button class="v6-filter-option" data-value="${cat.id}" onclick="selectFilterOption(this)">${escapeHtml(cat.name)}</button>`
      ).join('');
    ...
    if (dealOptions) {
      ...
      const html = uniqueDeals.map(deal =>
        `<button class="v6-filter-option" data-value="${deal}" onclick="selectFilterOption(this)">${escapeHtml(deal)}</button>`
      ).join('');
    ...
    if (sizeOptions) {
      ...
      const html = uniqueSizes.map(size =>
        `<button class="v6-filter-option" data-value="${size}" onclick="selectFilterOption(this)">${escapeHtml(size)}</button>`
      ).join('');
  ...
  function switchDateTab(tabName) {
    ...
    document.querySelectorAll('.v6-date-picker-tab').forEach(tab => {
      tab.classList.toggle('active', tab.dataset.tab === tabName);
    });

    // Update tab content visibility
    document.querySelectorAll('.v6-date-tab-content').forEach(content => {
  ...
  function selectWeek(weekId) {
    ...
    // Update UI selection
    document.querySelectorAll('.v6-week-option').forEach(el => {
  ...
  function applyDateSelection() {
    ...
    const customTab = document.querySelector('.v6-date-picker-tab[data-tab="custom"]');
  ...
  function switchEntityTab(tabName) {
    // Update tab button states
    document.querySelectorAll('.v6-entity-tab').forEach(tab => {
      tab.classList.toggle('active', tab.dataset.tab === tabName);
    });

    // Update tab content visibility
    document.querySelectorAll('.v6-entity-tab-content').forEach(content => {
      content.classList.toggle('active', content.id === `${tabName}-tab-content`);
    });
  }
  ...
  function renderGroupList(searchQuery = '') {
    ...
    if (filteredGroups.length === 0) {
      groupList.innerHTML = `
        <div class="v6-group-empty">
          <div class="v6-group-empty-icon">📁</div>
          <p class="v6-group-empty-text">${query ? 'No groups match your search' : 'No custom groups created yet'}</p>
        </div>
      `;
      return;
    }
    ...
    const groupsHTML = filteredGroups.map(group => {
      ...
      const iconClass = group.type === 'brand-group' ? 'v6-group-icon--subbrand-group' : 'v6-group-icon--store-group';
      const icon = group.type === 'brand-group' ? 'workspaces' : 'folder_special';

      return `
        <div class="v6-group-item ${isSelected ? 'selected' : ''}"
             data-group-id="${group.id}"
             onclick="selectGroup('${group.id}')">
          <div class="v6-group-icon ${iconClass}">
            <span class="material-symbols-outlined">${icon}</span>
          </div>
          <div class="v6-group-info">
            <div class="v6-group-name">${escapeHtml(group.name)}</div>
            <div class="v6-group-meta">${group.storeCount} stores · ${escapeHtml(group.description)}</div>
          </div>
          <div class="v6-group-check">
            <span class="material-symbols-outlined">check</span>
          </div>
        </div>
      `;
  ...
  function selectGroup(groupId) {
    ...
    document.querySelectorAll('.v6-entity-item').forEach(el => {
      el.classList.remove('selected');
    });
  }
  ...
  function openFilterModal() {
    if (elements.addFilterModal) {
      ...
      // Reset UI
      document.querySelectorAll('.v6-filter-type-btn').forEach(b => b.classList.remove('active'));
      document.querySelector('[data-type="category"]')?.classList.add('active');
      document.querySelectorAll('.v6-filter-value-select').forEach(s => s.classList.remove('active'));
      document.getElementById('category-options')?.classList.add('active');
      document.querySelectorAll('.v6-filter-option').forEach(o => o.classList.remove('selected'));
    }
  }
  ...
  function selectFilterOption(element) {
    const parent = element.closest('.v6-filter-options');
    parent.querySelectorAll('.v6-filter-option').forEach(o => o.classList.remove('selected'));
  ...
  function renderFilterChips() {
    ...
    const chipsHTML = state.activeFilters.map((filter, index) => {
      const colorClass = filter.type === 'category' ? 'v6-filter-chip--category'
                       : filter.type === 'deal' ? 'v6-filter-chip--deal'
                       : 'v6-filter-chip--size';
      ...
      return `
        <span class="v6-filter-chip ${colorClass}">
    ...
    const addButtonHTML = `
      <button class="v6-add-filter" id="add-filter-btn" aria-label="Add filter">
  ...
  function getPercentileBadgeHTML(percentile) {
    ...
    return `
      <div class="v6-percentile-badge">
        <span class="material-symbols-outlined v6-percentile-badge__icon">bar_chart</span>
        <span class="v6-percentile-badge__value v6-percentile-badge__value--${colorClass}">${percentile}</span>
      </div>
    `;
  }
  ...
  function getSortableHeaderHTML(label, column, filterable = false, filterType = null) {
    ...
    const activeClass = isActive ? 'v6-th-sort--active' : '';
    ...
    if (filterable && filterType) {
      ...
      filterHTML = `
        <select class="v6-th-filter" data-filter="${filterType}" onchange="window.__handleColumnFilter && window.__handleColumnFilter('${filterType}', this.value)">
    ...
    return `
      <th class="v6-th-sortable ${activeClass}" data-column="${column}">
        <div class="v6-th-content">
          <span class="v6-th-label" onclick="window.__handleColumnSort && window.__handleColumnSort('${column}')">${label}</span>
          <span class="v6-th-sort-icon material-symbols-outlined" onclick="window.__handleColumnSort && window.__handleColumnSort('${column}')">${sortIcon}</span>
          ${filterHTML}
        </div>
      </th>
    `;
  ...
  function getCategorySortableHeaderHTML(label, column, cssClass = '') {
    ...
    return `
      <th class="v6-th-sortable ${activeClass} ${cssClass}" data-column="${column}">
        <div class="v6-th-content">
          <span class="v6-th-label" onclick="window.__handleCategoryColumnSort && window.__handleCategoryColumnSort('${column}')">${label}</span>
          <span class="v6-th-sort-icon material-symbols-outlined" onclick="window.__handleCategoryColumnSort && window.__handleCategoryColumnSort('${column}')">${sortIcon}</span>
        </div>
      </th>
    `;
  }
  ...
  function getComparisonBadgeHTML(change) {
    ...
    return `
      <div class="v6-comparison-badge v6-comparison-badge--${changeType === 'up' ? 'increase' : changeType === 'down' ? 'decrease' : 'stable'}">
  ...
  function getComparisonArrowHTML(change) {
    ...
    return `
      <span class="v6-comparison-arrow v6-comparison-arrow--${cssClass}">
        <span class="material-symbols-outlined">${arrow}</span>
      </span>
    `;
  }
