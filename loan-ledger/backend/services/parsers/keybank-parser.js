/**
 * KeyBank Statement Parser
 * Extracts check and withdrawal information from KeyBank statements
 */

/**
 * Parse KeyBank statement text
 * @param {string} text - OCR or PDF extracted text
 * @returns {Object} Parsed transactions
 */
function parseKeyBank(text) {
  try {
    const result = {
      transactions: [],
      statementDate: null,
    ...
    // Find Paper Checks section
    const checksSection = extractSection(text, /Paper\s+Checks/i, /(?=\n[A-Z][a-z]+\s+[A-Z]|\n\s*Total)/i);
    if (checksSection) {
    ...
    const withdrawalsSection = extractSection(text, /Withdrawals/i, /(?=\n[A-Z][a-z]+\s+[A-Z]|\n\s*Total)/i);
    ...
    // Filter for loan payments (NAVIENT, MOHELA, or specific check numbers)
    ...
    console.error('KeyBank parser error:', error.message);
    throw new Error(`Failed to parse KeyBank statement: ${error.message}`);
  }
}
...
function parseChecksSection(text) {
  ...
  // Pattern: Check # | Date | Amount
  const checkRegex = /(\d+)\s+(\d{1,2}\/\d{1,2}(?:\/\d{2,4})?)\s+\$?([\d,]+\.?\d*)/gim;
